...
## Logistic Regression ##
# probability an observation belongs to a class
# produces a linear decision boundary
# ROC curve gives rate of a false/true positive
# AUC (area under ROC curve) quantifies the quality of the model; closer to 1 is better
...
# Still split and perform cross-validation on training set, but save test set for final tuned model evaluation
# Grid search is a common cross-validation method for searching over a span of hyperparameters, but does not scale well
# Radomized search tests a random set of hyperparameters (number set by n_iter), which provides good tuning while minimizing the need to try every parameter combo
