...
### Key-Value Cache for Search Results
- Same architecture as WebCrawler (without crawler portion)
- Query API:
```
...
- Update cache when:
    ...
    - Pages are removed or added
    - Page rank changes
    - Use TTL
- Shard cache across cluster using consistent hashing scheme

### Amazon Sales Rank Service
- Sales Rank Service:
    - Sales (Write) API stores raw transaction logs on S3 Object Store
...
- MapReduce Implementation:
    ...
    - Perform a distributed sort
        - By reformatting the key, mapreduce will automatically sort by key
        - To get in descending order (by most purchased), do a negative quantity
```
